...
import inkex, math
import inkex.localization as localization
from inkex.paths import CubicSuperPath, Path, Move
...
class AndroidVector(inkex.Effect):
    ...
    def effect(self):
        ...
        vector.set(_ns('width'), str(self.svg.uutounit(self.__unittouu(width), 'px')) + 'dp')
        vector.set(_ns('height'), str(self.svg.uutounit(self.__unittouu(height), 'px')) + 'dp')
    ...
    def save(self, stream):
        ...
        self.etree.write(stream, pretty_print=True)
    ...
    def _parse_child(self, src, ancestors):
        ...
        if tag == 'g':
            ...
            if 'style' not in src.attrib:
                ...
                style = dict(Style.parse_str(src.get('style')))
